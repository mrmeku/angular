  ...
  describe('animation tests', function() {
    ...
    describe('animation listeners', () => {
      ...
      it('should fire callbacks on a sub animation once it starts and finishes', () => {
        ...
        p1.finish();
        p2.finish();
        p3.finish();  // this is a cover player
        p4.finish();

        expect(cmp.log).toEqual(['parent-start', 'child-start', 'child-done', 'parent-done']);
      });
      ...
      it('should fire callbacks and collect the correct the totalTime and element details for any queried sub animations',
        ...
        class Cmp {
           ...
           const players = getLog();
           // 1 + 4 + 4 = 9 players (plus 4 cover players for each ngFor sub animation)
           expect(players.length).toEqual(13);

           const
               [pA, pq1a, pq1b, pq1c, pq1d, pq2a, pq2aCover, pq2b, pq2bCover, pq2c, pq2cCover, pq2d,
                pq2dCover] = getLog();
           pA.finish();
           pq1a.finish();
           pq1b.finish();
           ...
           expect(cmp.log).toEqual(['c-0-done', 'c-1-done', 'c-2-done', 'all-done', 'c-3-done']);
