  ...
  describe('animation query tests', function() {
    ...
    describe('query() and queryAll()', () => {
      ...
      it('should persist inner sub trigger styles once their animation is complete', () => {
        ...
        expect(p1.keyframes).toEqual([{height: '0px', offset: 0}, {height: '444px', offset: 1}]);

        p1.finish();
        p2.finish();
        expect(p1.element.style.height).toEqual('444px');
      });
    ...
    describe('sub triggers', () => {
      ...
      it('should run and operate a series of triggers on a list of elements with overridden timing data',
         () => {
          ...
          `,
             animations: [
               trigger('parent', [transition(
                                     ...
                                     [
                                       ...
                                       queryAll('.item', [animateChild('2.5s 500ms')]),
                                       animate(1000, style({opacity: '0'}))
                                     ])]),
               trigger('child', [transition(
                                    ':enter',
           ...
           const
               [pA, pc1, pc1cover, pc2, pc2cover, pc3, pc3cover, pc4, pc4cover, pc5, pc5cover, pZ] =
                   players;
        ...
        @Component({
          ...
          animations:
              [trigger('parent', [transition(
                                     ...
                                     [
                                       style({opacity: 0}), animate(1000, style({opacity: 1})),
                                       queryAll('.child', [animateChild('1s')]),
                                       animate(1000, style({opacity: 0}))
                                     ])])]
        })
        class Cmp {
      ...
      it('should animate all sub triggers on the element at the same time', () => {
        ...
        const players = getLog();
        expect(players.length).toEqual(5);
        const [pA, pc1, pc2, pcover, pZ] = players;
